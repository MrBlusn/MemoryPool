...
const std::chrono::milliseconds CentralCache::DELAY_INTERVAL{1000};     // 定义延迟归还的时间阈值：1000 毫秒（1 秒）
...
CentralCache::CentralCache()
{
    ...
    {
        ptr.store(nullptr, std::memory_order_relaxed);   //初始化所有大小类的空闲链表头为 nullptr
    }
    for (auto& lock : locks_)
    {
        lock.clear();                   // 初始化所有大小类的锁为未锁定状态
    }
    // 初始化延迟归还相关的成员变量
    for (auto& count : delayCounts_)
    {
        count.store(0, std::memory_order_relaxed);  // 初始化每个大小类的延迟计数为 0
    }
    for (auto& time : lastReturnTimes_)
    {
        time = std::chrono::steady_clock::now();    // 初始化每个大小类的最后归还时间为当前时间
    }
    spanCount_.store(0, std::memory_order_relaxed); // 初始化span计数为 0
}

void* CentralCache::fetchRange(size_t index)
{
    // 索引检查，当索引大于等于FREE_LIST_SIZE时，说明申请内存过大应直接向系统申请
    // 超出范围说明请求过大，应由 ThreadCache 直接调用 malloc
...
SpanTracker* CentralCache::getSpanTracker(void* blockAddr)
{
    ...
    size_t trackerCount = spanCount_.load(std::memory_order_relaxed);
    size_t scanLimit = std::min(trackerCount, spanTrackers_.size());
